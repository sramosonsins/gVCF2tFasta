...
@test "vcf2tfa ok" {
  ...
  assert_success
  [ "${lines[0]}" = "vcf2tfa" ]
  [ "${lines[2]}" = "Usage: vcf2tfa -v input.vcf(.gz) -r reference.fa(.gz) -c ploidy [-o outputname] [-n chromosomes.txt] [-i 0/1]" ]
}
...
@test "Convert a VCF" {
  ...
  # echo ---------------------------------------------------------
  run vcf2tfa -v  $TEST_FILES_DIR/example.vcf -r  $TEST_FILES_DIR/ref.fa.gz -c 2 -o $TEST_OUTPUT/example -n  $TEST_FILES_DIR/ref.fa.fai 
  assert_success
...
@test "Convert a compressed VCF 1" {
  ...
  # echo --------------------------------------------------------------------
  run vcf2tfa -v  $TEST_FILES_DIR/example.vcf.gz -r  $TEST_FILES_DIR/ref.fa.gz -c 2 -o $TEST_OUTPUT/example -n  $TEST_FILES_DIR/ref.fa.fai 
  assert_success
  assert_file_exist $TEST_OUTPUT/example.tfa.gz
}
...
@test "Convert a multiple VCF with SNPs of 2 pop" {
  ...
  # echo -----------------------------------------------------------------------------------------------------------
  run vcf2tfa -v $TEST_FILES_DIR/multipleVCF_2pop.vcf.gz -r $TEST_FILES_DIR/ref.fa.gz -c 2 -o $TEST_OUTPUT/example_mult -n $TEST_FILES_DIR/ref.fa.fai
  assert_success
...
@test "Convert a VCF with SNPs pool" {
  ...
  # echo ---------------------------------------------------------------------------------
  run vcf2tfa -v $TEST_FILES_DIR/pool_p10.vcf.gz -r $TEST_FILES_DIR/ref.fa.gz -c 10 -o $TEST_OUTPUT/pool -n $TEST_FILES_DIR/ref.fa.fai
  assert_success
...
@test "Convert a compressed VCF 2" {
  ...
  # echo ---------------------------------------------------------------------------------
  run vcf2tfa -v $TEST_FILES_DIR/gatk_combined.vcf_10lines.recode.vcf.gz -r $TEST_FILES_DIR/pdulcis26.contigs_Pd1-8.fa -c 2 -o $TEST_OUTPUT/TEST_almond_10lines -i 0 -n $TEST_FILES_DIR/pdulcis26.chromosomes.lengths
